...
describe('RotateRoad data structure Testing', function() {
    it('full test cases:', function() {
        assert.equal(obj2.fullPhrase, 'rotateStreet(yStreet, coordinate(660, 225), coordinate(710, 275),coordinate(595, 474),coordinate(539, 441)).');
    });


    it('fluent test:', function() {
        assert.equal(obj2.fluent, 'rotateStreet');
    });

    it('name test:', function() {
        assert.equal(obj2.name, 'yStreet');
    });

    it('X Y test:', function() {
        assert.equal(obj2.X1, 660);
        assert.equal(obj2.Y1, 225);
        assert.equal(obj2.X2, 710);
        assert.equal(obj2.Y2, 275);
        assert.equal(obj2.X3, 595);
        assert.equal(obj2.Y3, 474);
        assert.equal(obj2.X4, 539);
        assert.equal(obj2.Y4, 441);
    });
